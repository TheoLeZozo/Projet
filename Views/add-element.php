...
$valImg  = $isEdit ? $element->getUrlImg() : '';
?>

<div class="page-shell">
  <h1 class="page-title"><?= $isEdit ? 'Edit Element' : 'Add Element' ?></h1>

  <div class="form-card">
    <form class="perso-form" method="post" action="index.php?action=<?= $isEdit ? 'edit-element' : 'add-element' ?>">
      <?php if ($isEdit): ?>
        <input type="hidden" name="id" value="<?= (int)$element->getId() ?>">
      <?php endif; ?>
...
>>>>>>> d069e895e7b001512b0a65d51dca5cc0fa835f95
  ...
  <h2 class="page-title" style="font-size:22px;margin-top:26px;">Existing Elements</h2>

  <div class="form-card">
    <?php if (empty($elements ?? [])): ?>
      <p class="perso-show-empty">No elements yet.</p>
    <?php else: ?>
      <div class="perso-sheet-grid" style="grid-template-columns:1fr 1fr;">
        <?php foreach (($elements ?? []) as $e): ?>
          <div class="meta-box">
            <div class="meta-value">
              <img class="meta-icon" src="<?= $this->e($e->getUrlImg()) ?>" alt="">
              <?= $this->e($e->getName()) ?>
              <span style="opacity:.7">#<?= (int)$e->getId() ?></span>
            </div>

            <div class="perso-show-actions" style="margin-top:12px;">
              <a class="btn" href="index.php?action=edit-element&id=<?= (int)$e->getId() ?>">Edit</a>
              <a class="btn btn-danger"
                 href="index.php?action=del-element&id=<?= (int)$e->getId() ?>"
                 onclick="return confirm('Delete this element?');">Delete</a>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
